...
import {
  useSteppedCalibration,
  FiducialSet,
} from "@/hooks/useSteppedCalibration";
...
interface SteppedCalibrationProps {
  config: any;
...
export const SteppedCalibration: React.FC<SteppedCalibrationProps> = ({
  config,
  updateConfig,
  cameraOptions,
  sourcePaths,
}) => {
  const calibration = useSteppedCalibration(cameraOptions, sourcePaths);
  ...
  // ---- Fiducial markers for overlay (only on the datum frame) ----
  ...
  const setAsActiveMethod = useCallback(async () => {
    try {
      const res = await fetch("/backend/update_config", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ calibration: { active: "stepped" } }),
      });
      const json = await res.json();
  ...
  return (
    <div className="space-y-6">
      <Card>
        <CardHeader>
          <CardTitle>Stepped Board (Pinhole)</CardTitle>
          <CardDescription>
            Per-camera sequence calibration using BOTH Z levels of a dual-level dot target.
            The two planes break the focal-length / depth ambiguity in a single rigid board.
          </CardDescription>
        </CardHeader>
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  {modelType === 'polynomial3d'
                    ? 'DaVis poly: one view, both levels'
                    : 'OpenCV pinhole across all frames'}
                </p>
              </div>
            </div>
          </div>
              ...
              <div className="p-3 border rounded space-y-3">
                ...
                <div className="grid grid-cols-[auto_1fr] gap-x-4 gap-y-1 items-center max-w-sm">
                  ...
                  {activeSequencePoses.map((pose) => {
                    ...
                    const isLabeling = labelTarget?.frameIdx === pose.frame_idx;
          ...
          {validation?.valid && (
            <div className="border-t pt-4">
              <CalibrationImageViewer
                key={`stepped-${camera}-${sourcePathIdx}`}
                backendUrl="/backend"
                    ...
                    {Object.entries(multiCameraJobStatus.camera_results)
                      .map(([cam, res]: [string, any]) => (
                        <span key={cam} className="mr-3">
                          {cam}: {res.status === 'completed' && res.rms != null
      ...
      {hasActiveModel && activeCameraModel && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              {activeCameraModel.model_type === 'polynomial3d'
                ? `3D polynomial model (single datum view) for Camera ${camera}`
                : `Pinhole calibration model for Camera ${camera}`}
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="grid md:grid-cols-2 gap-6">
              {activeCameraModel.model_type === 'polynomial3d' ? (
                <div className="space-y-2">
                  <h4 className="text-sm font-semibold mb-2">Per-Plane Reprojection RMS</h4>
                  <div className="grid grid-cols-2 gap-2 text-sm">
                    {(activeCameraModel.plane_rms ?? []).map((v, i) => (
                      <div key={i}>
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    World&rarr;image cubic (degree-3 in X,Y &times; degree-1 in Z); no intrinsics.
                  </p>
                </div>
              ) : (
                <div className="space-y-2">
            ...
            <CalibrationFigureGallery
              query={{ board: "stepped", camera, source_path_idx: sourcePathIdx }}
              trigger={activeCameraModel}
            />
          </CardContent>
        </Card>
      )}
